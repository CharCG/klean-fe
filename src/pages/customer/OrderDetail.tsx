...
import { AlertTriangle, Star, FileText, CreditCard, CheckCircle2, Clock4, Package, Clock, Truck, CheckCircle } from 'lucide-react';
...
export default function CustomerOrderDetail() {
  ...
  const reportMutation = useMutation({
    ...
    onError: (err) => {
      showToast(axios.isAxiosError(err) ? err.response?.data?.message ?? 'Failed to submit report' : 'Something went wrong', 'error');
    },
  });
  ...
  const confirmMutation = useMutation({
    ...
    onError: (err) => {
      showToast(axios.isAxiosError(err) ? err.response?.data?.message ?? 'Failed to confirm' : 'Something went wrong', 'error');
    },
  });
      ...
      <div className="p-6 space-y-5">
        ...
        {paymentStatus === 'PAID' && (
          ...
          >
            ...
            <div>
              <p className="font-semibold text-sm" style={{ color: 'var(--color-success)' }}>Payment Successful</p>
        ...
        {paymentStatus === 'PENDING' && !snapToken && (
          ...
          >
            ...
            <div>
              <p className="font-semibold text-sm" style={{ color: 'var(--color-warning)' }}>Awaiting Payment</p>
              <p className="text-xs mt-0.5" style={{ color: 'var(--color-text-secondary)' }}>
        ...
        {paymentStatus === 'PENDING' && snapToken && (
          <button
            onClick={handlePay}
            disabled={isPaying}
            className="flex items-center justify-center gap-2 w-full py-3.5 rounded-xl text-sm font-semibold transition-all cursor-pointer disabled:opacity-70"
            style={{
              backgroundColor: 'var(--color-primary)',
              color: 'var(--color-card)fff',
              boxShadow: isPaying ? 'none' : '0 4px 12px rgba(36,140,188,0.35)',
              border: 'none',
            }}
          >
            <CreditCard size={16} />
        ...
        <div
          ...
          style={{ backgroundColor: 'var(--color-card)', border: '1px solid var(--color-stroke)' }}
        >
          <h3 className="font-semibold text-sm mb-3 pb-2" style={{ color: 'var(--color-text)', borderBottom: '1px solid var(--color-stroke)' }}>
            Order Items
          </h3>
          <div className="space-y-2">
            ...
            <div className="flex justify-between pt-3 mt-2" style={{ borderTop: '1px solid var(--color-stroke)' }}>
              <span className="font-semibold text-sm" style={{ color: 'var(--color-text)' }}>Total</span>
        ...
        >
          ...
          <p><span style={{ color: 'var(--color-text-secondary)' }}>Fulfillment:</span> <span className="font-medium" style={{ color: 'var(--color-text)' }}>{order.fulfillment}</span></p>
          <p><span style={{ color: 'var(--color-text-secondary)' }}>ETA:</span> <span className="font-medium" style={{ color: 'var(--color-text)' }}>{order.estimationTime}</span></p>
          {order.notes && <p><span style={{ color: 'var(--color-text-secondary)' }}>Notes:</span> <span className="font-medium" style={{ color: 'var(--color-text)' }}>{order.notes}</span></p>}
        </div>
        ...
        {order.review && (
          <div className="rounded-xl p-5" style={{ backgroundColor: 'var(--color-card)', border: '1px solid var(--color-stroke)' }}>
            <h3 className="font-semibold text-sm mb-3 pb-2" style={{ color: 'var(--color-text)', borderBottom: '1px solid var(--color-stroke)' }}>Your Review</h3>
            <div className="flex gap-1 mb-2">
              {[1, 2, 3, 4, 5].map((s) => (
                <Star key={s} size={16} className={s <= order.review!.rating ? 'fill-yellow-500 text-yellow-500' : ''} style={{ color: s > order.review!.rating ? 'var(--color-stroke-medium)' : undefined }} />
              ))}
            </div>
            <p className="text-sm" style={{ color: 'var(--color-text-secondary)' }}>"{order.review.comment}"</p>
          </div>
        )}
        ...
        {showReportForm && (
          <form onSubmit={handleReport} className="rounded-xl p-5 space-y-4" style={{ backgroundColor: 'var(--color-card)', border: '1px solid var(--color-stroke)' }}>
            <h3 className="font-semibold text-sm" style={{ color: 'var(--color-text)' }}>Describe the issue</h3>
            <textarea
              name="issue"
              required
              rows={4}
            ...
            <div className="flex gap-3">
              <Button type="button" onClick={() => setShowReportForm(false)} variant="outline" className="flex-1">Cancel</Button>
              <Button type="submit" isLoading={reportMutation.isPending} variant="danger" className="flex-1">Submit</Button>
            </div>
          </form>
        )}
        ...
        {order.report && (
          <div className="rounded-xl p-5" style={{ backgroundColor: 'var(--color-card)', border: '1px solid var(--color-stroke)' }}>
            <div className="flex items-center gap-2 mb-2">
              ...
              <h3 className="font-semibold text-sm" style={{ color: 'var(--color-text)' }}>Reported Issue</h3>
            </div>
            <p className="text-sm leading-relaxed" style={{ color: 'var(--color-text-secondary)' }}>{order.report.issue}</p>
            <span
              className="text-[10px] font-semibold px-2 py-1 rounded-full uppercase mt-2 inline-block"
              style={{
                backgroundColor: order.report.status === 'OPEN' ? 'var(--color-danger-light)' : 'var(--color-success-light)',
